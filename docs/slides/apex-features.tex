...
\begin{frame}{The six screens}{Everything below is a live screenshot of the current build}
  ...
  \begin{tabular}{@{}L{25mm}L{42mm}L{55mm}@{}}
    ...
    Session Debrief & Everyone    & Whole-run measurements and written coaching \\
    ...
    Study Results & Researcher    & Paired participant results and analysis-ready CSV export \\
    \bottomrule
  \end{tabular}
  \par\vspace{0.9em}
  {\footnotesize\color{cgray}Study Results is hidden unless
  \code{APEX\_RESEARCH\_MODE=1}; participants never see it.}
\end{frame}
...
\begin{frame}{Driver debrief --- where the time actually went}
  ...
  the debrief (``5.32\,s off 0822-coached-lap02. 3.86\,s of it went at T4, T1, T5''), a delta column and the
  reference trace in purple. The sentence names whichever lap was picked to compare with, which is not
  always the session best --- the picker marks which lap is. Each point pairs a measured
  time loss with the largest measured difference over the same stretch --- and stops
  short of claiming the second caused the first.}
\end{frame}
...
\begin{frame}{Engineering status}{Where the build stands today}
  ...
  \begin{column}{0.48\textwidth}\small
    ...
    \begin{itemize}\itemsep2pt
      ...
      \item Local Granite 4.1 integration end to end for post-lap review, with
...
\begin{frame}{Note on the screenshots}{So the evidence in this deck is itself auditable}
  ...
  \begin{itemize}\itemsep4pt
    ...
    \item The coaching content on the Analysis slide is a
          \kw{real response} from the local \code{ibm-granite/granite-4.1-3b}
          server, not a mock or a canned reply.
    \item The Garage, Analysis, Debrief and Compare screens use the bundled
          five-lap sample session, which ships with the application.
    \item The Replay screen uses a real captured drive, because that is the data
          that carries world position.
    \item The Collect Data screen is real but shown in its completed state;
          running it for the screenshot would have required a live TORCS session.
  \end{itemize}
\end{frame}
