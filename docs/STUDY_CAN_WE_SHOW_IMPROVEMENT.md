...
### 11.10 最严重的一条：研究者自己的浏览被记成了参与者的剂量（2026-08-27 下午）

给导师做幻灯片、顺手看了一眼真实 workspace，发现多了一个 `~/Apex/exposure/` 目录，
里面 **16 条 view**，挂在 B0826 和 C0826 名下。时间戳全是 **11:31–11:46** —— 也就是
**你自己**上午用新版 Apex 点参与者的圈的那段时间。

这是 11.2 那个守卫没想到的一面。`is_recordable()` 只挡了 sample session，但
**在研究者的分析机上，每一个 session 都是收进来的参与者数据**，点开它是分析，不是
在接受辅导。而这台机器恰恰是最后导出剂量列的那一台 —— 假数据会直接进回归。

修法：`register()` 和 GUI 导入现在都往 session 里写一个 `arrived.json`（参与者 id、
来自哪个包、什么时候收的），`is_recordable()` 见到它就不记。判据是对的：**从 handover
收进来的 session，本来就是在别人机器上开的。**

已经做的清理：
- 五个真实 session 都补写了 `arrived.json`（追溯，注明是 2026-08-27 补的）；
- 那 16 条假记录移到 `~/Apex/exposure-quarantined-20260827/`，**没有删**，可以回看；
- `racecoach study-exposure` 现在重新回到"什么都没记录"，`study-summary` 的五个剂量列
  重新是**空白** —— 这才是对的：那五个包是这个功能之前采的。

两条测试红过再绿：一条断言带 `arrived.json` 的 session 记 0 条，另一条断言**没有**这个
标记的（也就是参与者自己机器上的）照记不误 —— 守卫不能把测量在真正需要它的地方也关掉。

> **注意**：你机器上正开着的那个 Apex 还是旧的，还在记。换过的版本要**重启 Apex** 才生效。
