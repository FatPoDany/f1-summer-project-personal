...
## 15. 把天花板抬起来：试过了，没抬成（2026-08-31 → 09-01）
...
>
> **三条里最后只留下一条。** 阈值标定（§15.2）和跨圈不重复（§15.4）留下了；
> 数字白名单连同它要解锁的那件事——让模型的措辞上屏——在拿真权重量过之后**撤回**了，
> 理由在 §15.6。这一节保留失败的那半边，因为下次有人再想抬这个天花板，
> 需要知道它为什么抬不动。
>
...
这本来是这批改动里唯一改变参与者读到的**文字来源**的一条，也是唯一被撤回的一条。
实现过、测试过、拿真权重跑过，然后删了。留在这里是因为结论本身有价值。

**做的是什么。** 原来 `_ground_model_prose` 在校验**之前**无条件跑，把
focus/issue/cause/action 全换成模板；配合 `_require_measurement_free_prose`（prose 里
出现任何数字直接判失败），结果就是 §1 那个结论——模型一个字都没写到参与者眼前。
改成：先拿模型自己的措辞过一遍完整合同，过了就保留，没过才落模板；数字校验从一个都
不许有换成只能说这条 finding 自己引用的数（value、ref、两者之差、span，以及弯名里的
数字），按模型自己选的精度比较。

**为什么撤。** §15.6 有完整数据，一句话是：**白名单校验的是这个数有没有被引用，
不是这句话对不对。** 五圈实测，模型措辞上屏 0 次；而唯一一条三字段全干净、会原样上屏的
finding，把比参考更早刹车说成更晚，然后建议再早一点刹。它引的每个数都正确、都在自己
引用的证据里，白名单一个字拦不住。

**下界原本是安全的**——过不了就落模板，等于现状——所以撤回不是因为它会变差，是因为
**它能变好的那条路实测走不通，而走通的那一次是错的**。为一个观测到 0 次生效的路径
保留一个能让反向建议上屏的开关，在研究仪器上不划算。

**留下的是什么。** `_require_measurement_free_prose` 回到原样（连同这段经历写进它的
docstring，免得有人再试一遍），`report_from_llm_text` 回到无条件覆盖。
`test_the_model_s_own_wording_never_reaches_a_card` 用一个**模型确实引用了的**数字盯住
这个决定：任何按数字来源判断的方案都会放它过去，只有无条件覆盖能让它红。

### 15.4 跨圈不重复

提示词里关于数字和措辞的那几条**跟着 §15.3 一起撤了**——措辞既然一律用模板，让模型在
prose 上多花力气只会抬高它写错弯角的概率（§15.6 实测五圈里有四条写错弯）。
`PROMPT_VERSION` 仍然从 `coach-v4` 升到 **`coach-v5`**，因为提示词确实变了：多了下面这块。
...
**实测：机制通了，模型不听。** 用真权重跑了两组，同样五圈、同样其它条件，
只差 `prior_advice` 开或关：

| 组 | 第 2-5 圈发布的 finding | 其中重复了之前弯+指标的 |
|---|---|---|
| 开（现状） | 9 | **3** |
| 关（对照） | 9 | **1** |

审计记录证明文字确实进了提示词——第 4 圈的 prompt 里逐字有
`- T8: Try leaving the Down Arrow alone for a moment longer on the approach.`（来自第 3 圈），
它照样又发了一张 `brake_point @ T8`。所以这是一条**建议**，不是闸门。

一个 session、每组 9 条，样本远不足以说它有害；但足以撤回"同一条建议不再每屏都来"这个说法。
**它目前没有可测的收益。** 要真的不重复，得把已经给过的 (弯, 指标) 从 `opportunity_catalog`
里去掉——那是硬闸门，代价是后面的圈可能没卡可发，需要单独决定。
...
- **红过再绿**：把 `report_from_llm_text` 里的无条件覆盖去掉，
  `test_the_model_s_own_wording_never_reaches_a_card` 立刻红，而且红在正确的理由上
  （`issue must not contain numeric values`）；恢复后全绿。
- 白名单那一版第一次实现按字符串枚举写，**被自己的测试抓到一个真 bug**：`1595.0` 是被引用的
  精确值却被拒（枚举漏了尾随零）。这条现在只剩记录价值——白名单本身撤了。
- **`837 passed / 17 skipped`**（改前 827/17）。`ruff check src tests` 干净。
- 20→17、掉的三张全是 `min_speed`，是用发布路径跑出来的，不是估的。
- **拿真权重端到端跑了打包的这份代码**：五圈、11 条 finding、**0 条非模板措辞**、0 次失败，
  `prior_advice` 按 0→2→4→5→6 累积并在 6 上封顶。措辞这条保证在真模型上成立，不只在测试里。
- 打包出来的 exe **把代码读回来验过**：新增的 8 个标识全在，撤掉的 5 个全不在（含 `coach-v4`）。

### 15.6 拿真 Granite 跑过了，结论决定了 §15.3 撤回
...
阈值标定（§15.2）和跨圈不重复（§15.4）留下，模型措辞上屏和它带来的数字白名单一起撤回，
回到无条件用模板（§15.3）。按实测，参与者读到的字和今天完全一致，变的是
**卡片挑得更准、同一条建议不再每屏都来**。

另外两个选项和为什么没选：

- **加方向校验再放行。** 要靠关键词判早/晚、多/少，本身就脆；而且按上面第一件事，
  做完大概率仍然全落模板——为一个观测到 0 次生效的路径加一层脆弱的检查。
- **按当时的实现打包。** 五圈没有一条上屏，下界成立；但第二轮那条说明上屏是可能发生的，
  一旦发生就是给参与者看反向建议。研究仪器上不接受这个尾部风险。

值钱的部分从来不是让一个 3B 模型自己说话。
