...
- [x] ~~把圈数/车/对手/tire factor 也对齐~~ —— 见 §13，第三个预设 `ibmf1-practice-v5`
- [x] ~~采集完自动导入 + 自动打包 + 自动上传 + 自动开复盘~~ —— 见 §14
- [ ] 和队友对齐 P2 的三条上游改动
...
> **已被 §13 取代（2026-09-02）。** 下面这节记录的是 9-01 的决定：只对齐赛道。
> 实测下来那不够 —— 圈数、车、对手、tire factor 四项里补了一项，绝对指标照样不能合。
> 现在有第三个预设 `ibmf1-practice-v5`，四项全对齐，并且已经是默认。见 §13。

### 12.4 已定：两条都留，aalborg 仍是默认（2026-09-01，已被 §13 取代）

**决定：保留 aalborg，同时把 `g-track-1` 加进 Apex。** 不是二选一。

- `apex-study-v1` —— `aalborg`，**默认**。选它的理由写在 `study_presets()` 的
  docstring 里：CG Speedway 柏油缓冲区太大，实测有参与者跑出赛道边缘 14 米仍然
  零损伤，incident count 在那条赛道上没有任何区分力。
- `apex-study-speedway-v1` —— `g-track-1`，新增。给需要和队友那 77 场放在一起
  统计的场次用。

**除了赛道，两个预设完全一样** —— 同样 3 圈、同样 `car7-trb1`、同样的
damage/tire/wall-touch 设置。`verify-study-preset.sh` 里加了一条 `diff`
专门钉死这件事：把 speedway 那份的名字和赛道替换回去之后必须和 `apexstudy.xml`
逐字节相同。**换赛道只能换一件事**，否则参与者两场之间的对比就不是在比赛道了。

选择在 UI 上是**按参与者**给的，不是按场次：同一个参与者的 baseline 和 coached
必须在同一条赛道上，否则两场之间的差值毫无意义（这也正是 #24 在服务端拦下的情况）。

#### 光有同一条赛道还不够

即使都跑 `g-track-1`，和队友的数据仍有三处不同，绝对指标**还是不能直接混**：

| | 队友 | Apex |
|---|---|---|
| 圈数 | 2 | 3（更多圈 = 更多刷快圈的机会） |
| 车 | `car1-trb1` | `car7-trb1` |
| 对手 | human + berniw/bt/olethros 三台 | **human 单人** |
| tire factor | 0 | 1 |

其中**对手**这条最要命：Apex 是单人跑，`finish_position` 恒为 1；队友是 4 车，
名次分布是 {1:26, 2:9, 3:17, 4:25}。这四项没有一项是"加一张地图"能解决的 ——
每一项都会改变参与者实际经历的比赛，属于研究协议层面的决定。

**车这一项还有个实现约束**：TORCS 的车不是在 raceman 里指定的，而是在
`drivers/human/human.xml` 这个单一驱动配置里，全安装共用一份。所以
`TorcsStudyPreset.car_id` 现在只是**记录**而不是**决定**。真要按预设换车，
得像 `screen.xml` / `graph.xml` 那样在启动前把驱动配置写出来。

所以：同赛道让**百分比口径**的合并变得干净（配合 #24 的 `best_lap_gain_pct`），
但绝对秒数的合并仍然需要先把上面四项也对齐 —— 那是下一个决定，不在这次改动里。

#### 生效前提

`apexstudyspeedway.xml` 是随安装包发的。**参与者手上那个 1 GB 的研究版必须重新构建**
才会有第二个预设 —— 在那之前旧版本会正常运行、只显示一个 aalborg 选项，
UI 不会崩（`race_config.is_file()` 那道检查会挡住）。



---

## 13. 把四项差异一次补齐（2026-09-02）

§12.4 只对齐了赛道。补完之后重新读了一遍他们的 `practice.xml`，结论是**那一项不够**：
圈数 2 vs 3、车 `car1-trb1` vs `car7-trb1`、对手 3 台机器人 vs 单人、tire factor 0 vs 1，
**四项里任何一项单独存在，绝对指标就不能合并**。补了一项等于没补。

所以新增第三个预设，四项全部按他们的来。

| | 队友 `ibmf1-practice-v5` | Apex `ibmf1-practice-v5`（新） | Apex 原有两个 |
|---|---|---|---|
| 赛道 | g-track-1 | g-track-1 | aalborg / g-track-1 |
| 圈数 | 2 | **2** | 3 |
| 车 | car1-trb1 | **car1-trb1** | car7-trb1 |
| 对手 | berniw(4) + bt(4) + olethros(5) | **同上** | 无 |
| tire factor | 0 | **0** | 1 |
| restart | yes | **yes** | no |
| maximum dammage | 不设（TORCS 默认） | **不设** | 0（禁用退赛） |

**这是新的默认预设。** 理由：在别的条件下采到的数据只能和 Apex 自己那五场比，
而不能和他们那 77 场放在一起。原来两个预设**原样保留**，因为已经采到的数据认它们的 id，
`study_preset_by_id` 对不认识的 id 是报错而不是替换。

### 13.1 "对齐"是一条会红的断言，不是文档里的一句话

`tests/test_team_preset.py` 读**两个文件**逐项比对：赛道、圈数、type、starting order、
restart、display mode/results、distance、fuel/damage/tire factor、起跑格设置、
整条发车顺序（模块 + idx）、focused driver。他们的 `practice.xml` 原样 vendored 在
`integrations/torcs-1.3.9/reference/`，blob 哈希 `23f6dea8…` 与他们仓库逐字节一致
（`.gitattributes` 里 `-text` 钉住,免得 checkout 时被改行尾）。

不 vendored 就得要求他们的仓库在场才能跑，那样 CI 里就不会跑，而**不跑的测试是注释**。
也支持 `IBMF1_PRACTICE_XML` 指向活文件，直接对他们的工作副本验。

**一处刻意不一致，两边都断言了**：两个文件都不设 `maximum dammage`。Apex 自己的预设禁用退赛
（新手跑不完就没数据），他们不设，那就用 TORCS 的默认。在这个预设里"和他们一样"比"带上我们的保护"更重要。
断言是双向的 —— 他们哪天设了，这条会红，然后我们跟着设。

### 13.2 车这次是真的换了，不再只是记录

TORCS 不从 raceman 读人类玩家的车，从 `drivers/human/human.xml` 读，全安装共用一份。
所以原来 `TorcsStudyPreset.car_id` 只是**记录**。现在启动前按预设把这个 profile 写出来
（和 `screen.xml` / `graph.xml` 同一时机），**profile 不存在时也在这里生成**而不是留给 TORCS ——
TORCS 是在我们跑完之后才写的，那恰好就是全新安装、第一次使用、参与者马上要开一台和出厂默认不同的车的那一刻。

### 13.3 格子里有几台车,现在进 manifest 也进 UI

`TorcsStudyPreset.opponents` 是新字段。**带对手跑的比赛不是 Apex 一直在采的那种比赛**：
`finish_position` 不再恒等于 1，圈间离散度变宽的原因也不再只是车手。
一份不说明自己是哪一种的采集，两边都合不进去。

### 13.4 队友仓库已经重构过目录

本文档里的路径按 2026-08-29 的布局写。他们在 `df93842` "Reorganize repository by runtime
ownership" 之后把 `player_kit/` 挪到了 `apps/player-kit/`。读他们源码时按新路径找，
本文档其余部分未逐条改写。

### 13.5 光有预设不够，参与者手上那个包要重建

`apexibmf1.xml` 随安装包发。**旧版本的研究包不会有第三个预设** ——
`race_config.is_file()` 那道检查会挡住，UI 不崩，只是少一个选项。
`build-windows.ps1` 的 `$required` 里加了这个 XML、`car1-trb1` 的两个资源、
以及 bt / olethros 两个机器人 DLL —— 少任何一个都是"比赛起不来"，
不如在构建时就失败。实测这三样在现装的 runtime 里都在。

## 14. 赛后闭环（2026-09-02）

队友的闭环是"比赛—上传—复盘—指导—下一场"。Apex 每一段都有,**接缝一个都没有**：
采完要参与者看一段说明、在保存对话框里选个文件夹，研究者事后再手工跑三条命令。
每一处都是环断掉的地方。

`racecoach.telemetry.handoff.finish_session` 是那个接缝，一个函数，
CLI 和 GUI 共用（两份会漂，而且只有一份会被测）：

```
racecoach finish <capture>            单独跑
racecoach capture-human --finish      比赛结束就跑
桌面版                                 完成页出现的那一刻自己跑，每步实时写在页面上
```

四步：**读进研究路径 → 存出要交的那个文件 → 翻译并上传 → 打开复盘**。

### 14.1 两条规矩

**任何一步都不能让参与者丢数据。** 跑到这里时比赛已经落盘、圈也已经在 Garage 里。
所以每一步各自隔离：上传失败不能连累"那个能把数据带出这台机器的文件"，
任何失败都以结果返回而不是抛进参与者正看着的窗口。

**上传不能是"不小心发生的"。** 上传是把人体研究数据放到第三方主机上，
所以只在**有人刻意放了 key** 的机器上发生。没有 key 不是错误，是"这台机器没被配置成要发"，
而那正是它该有的默认。

### 14.2 本机采的数据，以前根本进不了 Study Results

Garage 一采完就显示，但 study-summary / study-laps / Study Results 读的是
`sessions_root` 里的 canonical 单圈，而**唯一往那里写的只有 `collect`，
那是分析者机器上跑的命令**。于是一场比赛：驾驶它的人看得见，同一台电脑上的研究看不见，
要等数据经过一次 zip 往返才补上。

`register_capture` 补掉这个。**不 adopt、不写 `arrived.json`** ——
那个标记的意思是"在别处驾驶的"，它是防止研究者读复盘被算成参与者剂量的那道闸。
另外 `mark_arrived` 现在也会拒绝给"本机采集过"的 session 打标记，
所以自己打包自己再 collect 一次（检查包能不能打开，是个合理动作）不会把它改标成别人的。

### 14.3 key 从他们的安装包配置文件读

参与者的笔记本不会有环境变量，以后也不会有。他们的 `coach-endpoint.json`
每份安装包里都是同一个 key，他们自己的部署文档白纸黑字写着这个 token 不是 secret。
所以研究者可以把拿到的那份文件直接放在 Apex 可执行文件旁边。

环境变量仍然优先；生产 import 凭据是另一个 header，**依旧从不读取**；
本仓库里依旧没有任何位置写过 key。

### 14.4 只有 coached 才自动开浏览器

他们自己的无 coach 版就是"照常采集，跑完不打开 review"。
给其他任何组的参与者打开那个页面，等于把研究正在对他隐藏的东西递到他手上 ——
不管服务器最后会显示什么。而且自 PR #15 起，服务器在研究者指派之前本来也什么都不给，
所以 CLI 和完成页都会把要指派的那一行打出来。

### 14.5 顺带修掉一个更早的泄漏

接线时发现：**测试套件在往真实的 `~/Apex` 工作区写东西**。跑一次全套会在
`~/Apex/coaching` 留下一条 coaching 审计记录，和参与者的躺在一起，形状一模一样，
唯一的区别是它写于没有人在驾驶的时刻 —— 下游没有任何东西能把它们分开，
而审计轨迹正是这个研究要测的东西之一。

原因是有 worker 活得比它的测试长（coaching 队列跑在 pool 上），
等它写的时候 monkeypatch 已经把环境变量还原了。现在在 session 作用域上加了一层"地板"，
迟到的写入有个无害的地方可去。接线本身一度让情况更糟（往开发者家目录写真 zip），
两个都堵上了，并且都有断言。
