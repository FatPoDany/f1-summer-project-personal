...
const int kMaximumHumanDrivers = 10;
const double kRadiansPerSecondToRpm = 60.0 / (2.0 * 3.14159265358979323846);
ApexHumanTelemetryWriter gWriters[kMaximumHumanDrivers];
bool gWriteErrorReported[kMaximumHumanDrivers] = {false};

bool validIndex(int index) { return index >= 1 && index <= kMaximumHumanDrivers; }
...
void fillTrack(ApexHumanTelemetrySample *sample, tCarElt *car, tTrack *track) {
...
void fillWheels(ApexHumanTelemetrySample *sample, tCarElt *car) {
    for (int wheel = 0; wheel < 4; ++wheel) {
        ApexWheelTelemetry &target = sample->wheels[wheel];
...
void ApexHumanTelemetryStart(int index, tCarElt *, tSituation *, tTrack *) {
    if (!validIndex(index)) return;
    ApexHumanTelemetryStop(index);
    gWriteErrorReported[index - 1] = false;
    const char *directory = std::getenv("APEX_HUMAN_TELEMETRY_DIR");
    if (directory == NULL || directory[0] == '\0') return;
    try {
        if (!gWriters[index - 1].open(directory, index)) {
            std::fprintf(stderr, "apex telemetry: cannot write to %s\n", directory);
            gWriteErrorReported[index - 1] = true;
        }
    } catch (...) {
        gWriters[index - 1].close();
        std::fprintf(stderr, "apex telemetry: recorder initialization failed safely\n");
        gWriteErrorReported[index - 1] = true;
    }
}
...
void ApexHumanTelemetryRecord(int index, tCarElt *car, tSituation *s, tTrack *track) {
    ...
    try {
        ...
        if (!gWriters[index - 1].write(sample) && !gWriteErrorReported[index - 1]) {
            std::fprintf(stderr, "apex telemetry: write failed; recorder disabled safely\n");
            gWriteErrorReported[index - 1] = true;
            gWriters[index - 1].close();
        }
    } catch (...) {
        if (!gWriteErrorReported[index - 1])
            std::fprintf(stderr, "apex telemetry: sample failed; recorder disabled safely\n");
        gWriteErrorReported[index - 1] = true;
        gWriters[index - 1].close();
    }
}
...
void ApexHumanTelemetryStop(int index) {
    ...
    try {
        ...
        std::fprintf(stderr, "apex telemetry: close failed safely\n");
    }
}
