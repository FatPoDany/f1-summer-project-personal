...
const char *kSchemaVersion = "apex-human-v1";
const char *kWheelNames[4] = {"fr", "fl", "rr", "rl"};

}  // namespace

ApexWheelTelemetry::ApexWheelTelemetry()
...
ApexHumanTelemetrySample::ApexHumanTelemetrySample()
    : simTimeS(0.0), deltaTimeS(0.0), carIndex(0), raceType(0), raceLap(0),
      ...
      trackToRightM(0.0), trackToMiddleM(0.0), trackToLeftM(0.0), trackWidthM(0.0),
      normalizedTrackPosition(0.0), headingAngleRad(0.0), trackSegmentId(0),
      trackSegmentType(0), trackSegmentType2(0), trackSegmentLengthM(0.0),
      trackSegmentRadiusM(0.0), trackSegmentArcRad(0.0), trackSurfaceFriction(0.0),
      trackSurfaceRollResistance(0.0), trackSurfaceRoughness(0.0),
      trackSurfaceDamage(0.0), accelCmd(0.0), brakeCmd(0.0), steerCmd(0.0),
      clutchCmd(0.0), gearCmd(0), gear(0), fuelL(0.0), engineRpmRadS(0.0),
      engineRpm(0.0), positionXM(0.0), positionYM(0.0), positionZM(0.0),
      rollRad(0.0), pitchRad(0.0), yawRad(0.0), yawRateRadS(0.0),
      speedBodyXMps(0.0), speedBodyYMps(0.0), speedBodyZMps(0.0),
      speedWorldXMps(0.0), speedWorldYMps(0.0), speedWorldZMps(0.0), totalSpeedMps(0.0),
      accelBodyXMps2(0.0), accelBodyYMps2(0.0), accelBodyZMps2(0.0) {}

ApexHumanTelemetryWriter::ApexHumanTelemetryWriter()
    : openSequence_(0), sampleIndex_(0), rowsSinceFlush_(0) {}

ApexHumanTelemetryWriter::~ApexHumanTelemetryWriter() { close(); }

bool ApexHumanTelemetryWriter::open(const std::string &directory, int driverIndex,
                                    long timestamp, long processId) {
    close();
    if (directory.empty() || driverIndex < 1 || driverIndex > 10) return false;
    ...
    filename << "human-" << driverIndex << '-' << timestamp << '-' << processId << '-'
             << openSequence_ << ".csv";
    path_ = directory;
...
bool ApexHumanTelemetryWriter::write(const ApexHumanTelemetrySample &sample) {
    ...
    output_ << kSchemaVersion << ',' << sampleIndex_++ << ',' << sample.simTimeS << ','
            ...
            << sample.racePosition << ',' << sample.state << ',' << sample.damage << ','
            << sample.collision << ',' << sample.simCollision << ',' << sample.collisionCount
            << ',' << sample.currentLapTimeS << ',' << sample.lastLapTimeS << ','
            << sample.bestLapTimeS << ',' << sample.distFromStartM << ',' << sample.distRacedM
            ...
            << sample.speedWorldZMps << ','
            << sample.totalSpeedMps << ',' << sample.accelBodyXMps2 << ','
            << sample.accelBodyYMps2 << ',' << sample.accelBodyZMps2;
    for (int wheel = 0; wheel < 4; ++wheel) {
        const ApexWheelTelemetry &value = sample.wheels[wheel];
...
void ApexHumanTelemetryWriter::close() {
    if (output_.is_open()) {
        output_.flush();
        output_.close();
    }
...
const std::string &ApexHumanTelemetryWriter::path() const { return path_; }
...
void ApexHumanTelemetryWriter::writeHeader() {
    output_ << "schema_version,sample,sim_time_s,delta_time_s,car_index,car_name,"
               ...
               "track_to_start_m,track_to_right_m,track_to_middle_m,track_to_left_m,"
               "track_width_m,track_pos,angle_rad,track_seg_id,track_seg_name,track_seg_type,"
               "track_seg_type2,track_seg_length_m,track_seg_radius_m,track_seg_arc_rad,"
...
std::string ApexHumanTelemetryWriter::csvText(const std::string &value) {
